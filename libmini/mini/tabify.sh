...
if ("$option" == "") then
   echo "usage: tabify.sh -x|u|t|X|U|T [<file>]"
   echo " replaces all tabs in <file> with 4-space (-x) or 8-space (-X)"
   ...
   echo " whithout a specified file argument all .c/.cpp and .h files are treated accordingly"
   exit
endif
...
if ($file == "") then
   if ($tabify == 1) then
      find . \( -name \*.c -o -name \*.cpp -o -name \*.h \) -exec $app -x {} \;
   endif
   if ($tabify == 2) then
      find . \( -name \*.c -o -name \*.cpp -o -name \*.h \) -exec $app -t {} \; -exec $app -u {} \;
   endif
   if ($tabify == 3) then
      find . \( -name \*.c -o -name \*.cpp -o -name \*.h \) -exec $app -u {} \; -exec $app -t {} \;
   endif
   if ($tabify == 4) then
      find . \( -name \*.c -o -name \*.cpp -o -name \*.h \) -exec $app -X {} \;
   endif
